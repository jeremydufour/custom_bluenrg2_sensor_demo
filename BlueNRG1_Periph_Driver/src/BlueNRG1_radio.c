...
typedef struct
{
  volatile uint8_t Pack_count_Tx[5];
  volatile uint8_t Pack_count_Rcv[5];
  volatile uint8_t stat_config;
  /*volatile uint8_t rcv_fail_count;*/
  volatile uint8_t unmapped_chan;
  volatile BlueTransBlueStrctPtr_t txpoint_prev;     // 12-13
  volatile BlueTransBlueStrctPtr_t rcvpoint_prev;    // 14-15
  volatile BlueTransBlueStrctPtr_t txpoint;          // 16-17
  volatile BlueTransBlueStrctPtr_t rcvpoint;         // 18-19
  volatile BlueTransBlueStrctPtr_t txpoint_next;     // 20-21
  volatile uint8_t remap_chan;        // 22
  volatile uint8_t config;            //  23
  volatile uint32_t access_address;   // 24-27
  volatile uint32_t crc_init;         // 28-30
  /* volatile  uint8_t rcv_fail_trig;*/
  volatile uint8_t hop_incr;
  volatile uint8_t chan_remap[5];
  volatile uint8_t enc_iv[8];
  volatile uint8_t enc_key[16];
  volatile uint8_t conf_byte5;
} BlueGlobPerMaster;
...
#ifndef BLUEGLOB_BASE 
#define BLUEGLOB_BASE (_MEMORY_RAM_BEGIN_ + 0xC0U)	// Discuss with Michele to have BLUEGLOB_BASE defined in BlueNRG1.h
#endif
...
static volatile BlueGlob* const blueglob = (BlueGlob*) BLUEGLOB_BASE;
static volatile BlueGlobPerMaster* const bluedata = (BlueGlobPerMaster*) (BLUEGLOB_BASE+12);
...
* @param  StateMachineNo: state machine number in multi state. From 0 to 7.
* @param  EncryptFlagTx: DISABLE: TX encryption is turned OFF. ENABLE: TX encryption is turned ON.
*         This parameter can be: ENABLE or DISABLE.
* @param  EncryptFlagRcv: DISABLE: RX encryption is turned OFF. ENABLE: RX encryption is turned ON.
*         This parameter can be: ENABLE or DISABLE.
* @retval None
*/
...
void RADIO_SetReservedArea(ActionPacket *p)
{
  ...
  p->trans_packet.byte10 = (p->ActionTag & INC_CHAN) |
    (p->ActionTag & NS_EN) |
      (p->ActionTag & PLL_TRIG) | (p->ActionTag & TXRX);
...
uint8_t RADIO_MakeActionPacketPending(ActionPacket *p)
{
  ...
  if(RADIO_GetStatus(&dummyTime) == BLUE_IDLE_0)
  {
    ...
    bluedata->txpoint_prev = 0;
    bluedata->rcvpoint_prev = 0;    
    bluedata->txpoint_next = 0;
    
    p1= &p->trans_packet;
    bluedata->txpoint =  BLUE_STRUCT_PTR_CAST(p1);
    bluedata->rcvpoint = BLUE_STRUCT_PTR_CAST(p1);
    
    globalParameters.current_action_packet = p;
    bluedata->stat_config = p->trans_config; /* Transmision or recepttion for first packet */
